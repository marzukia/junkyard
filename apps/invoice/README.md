...
## Deployment
Part of the junkyard monorepo. Live at https://junkyard.mrzk.io/invoice/ . Deploy is the consolidated `scripts/build-site.sh` (run by `.github/workflows/deploy-pages.yml` on push to `main`), building this app with `--base=/invoice/` into `dist/invoice/`. Umami analytics are injected at build from repo-root `umami-ids.txt`.
