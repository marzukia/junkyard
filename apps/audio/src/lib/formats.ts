...
export const SAMPLE_RATES: SampleRate[] = ["original", 44100, 48000, 96000, 192000];
...
export const ACCEPTED_AUDIO = ".mp3,.wav,.flac,.m4a,.ogg,.opus,.aiff,.aac,.wma,.mp4a,.m2a";

export function isAudioFile(file: File): boolean {
  const ext = file.name.split(".").pop()?.toLowerCase() ?? "";
  const accepted = ACCEPTED_AUDIO.split(",").map((e) => e.replace(".", "").toLowerCase());
