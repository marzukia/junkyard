/**
 * Monotonic phase guard tests for captionStore.
 *
 ...
 * must succeed. The run handler (runCaptionPipeline) calls setPhase("idle")
 * first so this path is always open.
 */
import { beforeEach, describe, expect, it } from "vitest";
import { useCaptionStore } from "./captionStore";
import type { Phase } from "./captionStore";

function phase(): Phase {
  return useCaptionStore.getState().phase;
}
function sp(p: Phase) {
  useCaptionStore.getState().setPhase(p);
}

describe("captionStore monotonic phase guard", () => {
  beforeEach(() => {
  ...
  it("re-run from done: idle reset allows model-loading (done->idle->model-loading)", () => {
    ...
    sp("done");
    expect(phase()).toBe("done");
    // Re-run sequence: runCaptionPipeline calls setPhase("idle") then setPhase("model-loading")
    sp("idle");
    sp("model-loading");
    expect(phase()).toBe("model-loading");
  });
