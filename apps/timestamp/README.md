...
## Deployment
Part of the junkyard monorepo. Live at https://junkyard.sh/timestamp/ . Deploy is the consolidated `scripts/build-site.sh` (run by `.github/workflows/deploy-pages.yml` on push to `main`), which builds this app with `--base=/timestamp/` into `dist/timestamp/`. Umami analytics are injected at build from the repo-root `umami-ids.txt` (no hardcoded script tag).
