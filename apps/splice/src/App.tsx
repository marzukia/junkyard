...
export function App() {
  ...
  const [result, setResult] = useState<{ blob: Blob; name: string; url: string; size: number } | null>(null);
  ...
  const handleSplice = useCallback(async () => {
    ...
    try {
      ...
      const blob = await spliceVideos(clipFiles, `combined_${Date.now()}.mp4`, () => {
        // Progress handling
      });
      ...
      <main className="site-main">
        ...
        {result ? (
          <div className="card">
            ...
            <p>
              Your combined video is {result.size > 0 ? `${(result.size / (1024 * 1024)).toFixed(2)} MB` : ''}
            </p>
            <video src={result.url} controls className="video-preview" style={{ maxWidth: '100%', marginTop: '1rem' }} />
        ...
        {error && (
          <div className="error-banner" role="alert" style={{ marginTop: '1rem' }}>
            ...
            <button type="button" onClick={() => setError(null)} style={{ marginLeft: '0.5rem' }}>
              ×
            </button>
          </div>
        )}
