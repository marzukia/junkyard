...
## Deployment
Part of the junkyard monorepo. Live at https://junkyard.mrzk.io/resume/ . Deploy is the consolidated `scripts/build-site.sh` (run by `.github/workflows/deploy-pages.yml` on push to `main`), which builds this app with `--base=/resume/` into `dist/resume/`. Umami analytics are injected at build from the repo-root `umami-ids.txt` (no hardcoded script tag).
