...
45 free, 100% client-side web tools. Everything runs in your browser: no server, no upload, no account. Tools that used to be paywalled, freemium, or just annoying are here, for free, forever. Live at **https://junkyard.sh**.
...
|----------|-------|---------|
| image/media | 16 | [Image Converter](https://junkyard.sh/convert/), [QR Code](https://junkyard.sh/qr/), [Background Remover](https://junkyard.sh/bg/), [OG Image](https://junkyard.sh/og/) |
...
Single GitHub Pages site built from a monorepo. Each of the 45 tools is a self-contained Vite + React app under `apps/<slug>/`. Shared UI lives in `kit/`. The hub landing page is in `hub/`. Headless tool logic lives in `packages/core` (`@junkyard/core`).
...
Builds all 45 apps + the hub into `dist/`, ready for GitHub Pages. See [`DEPLOY.md`](DEPLOY.md) for the full pipeline.
